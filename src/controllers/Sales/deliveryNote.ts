...
export const getInvoiceNumber = async (req: Request, res: Response) => {
    try {
        ...
        if (voucher) {
            const invoiceNo = await nextInvoice(req.body.db_name, voucher.prefix || "", voucher.numberMethod, req.body.voucherId, "DELIVERY_NOTE");
...
export const insert = async (req: Request, res: Response) => {
    try {
        ...
        if (voucher.numberMethod === 'Manual') {
            ...
            const deliveryNote = await checkInvoiceNo(db_name, "DELIVERY_NOTE", req.body.invoiceNo);
            if (deliveryNote) {
                return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: 'Invoice number already exists' });
            }
        } else {
            invoiceNo = await nextInvoice(req.body.db_name, voucher.prefix || "", voucher.numberMethod, req.body.voucherId, "DELIVERY_NOTE");
        }
        ...
        for (let x in items) {
            ...
            if (items[x].isExpense) {
                ...
                itemData.push({
                    salesOrderId: items[x]?.salesOrderId || null,
                    invoiceDate: new Date(req.body.invoiceDate),
        ...
        const deliveryNote = await prisma.deliveryNote.create({ data: data });
        if (deliveryNote) {
            const itData = itemData.map((i: any) => { i.deliveryNoteId = deliveryNote.id; return i; });
            await prisma.deliveryNoteItems.createMany({ data: itData });
            await activityLog(db_name, "INSERT", "deliveryNote", req.body, req.cookies.id);
...
export const update = async (req: Request, res: Response) => {
    try {
        ...
        const prisma = getPrismaClient(db_name);
        const getDN = await prisma.deliveryNote.findFirst({ where: { id: req.body.id }, include: { items: { where: { isDelete: false } } } })
        if (!getDN) {
        ...
        for (let x in items) {
            ...
            if (items[x].isExpense) {
                ...
                itemData.push({
                    ...
                    discount: discount,
                    discountAmount: discountAmount,
                    discountType: items[x].discountType || null,
                    isInvoiceDiscount: req.body.discount > 0,
                    invoiceDiscount: invoiceDiscount,
                    invoiceDiscountAmount: invoiceDiscountAmount,
                    taxableAmount: taxable - invoiceDiscountAmount,
        ...
        if (deliveryNote) {
            ...
            if (diff.length > 0) {
                ...
                await prisma.deliveryNoteItems.updateMany({
                    where: { deliveryNoteId: req.body.id, id: { in: deleteItemIds } },
                    data: { isDelete: true }
                });
            }
            const itData = itemData.map((i: any) => { i.deliveryNoteId = req.body.id; return i; });
            for (let i in itData) {
                if (!itData[i]?.id && itData[i]?.id === 0) {
                    let dt: any = itData[i];
                    delete dt['id'];
                    await prisma.deliveryNoteItems.create({
                        data: dt
                    });
                } else {
                    await prisma.deliveryNoteItems.update({
                        where: { id: itData[i]?.id },
                        data: itData[i],
                    });
                }
            };
            await activityLog(db_name, "UPDATE", "deliveryNote", req.body, req.cookies.id);
            return res.json({ st: true, statusCode: StatusCodes.OK, msg: "delivery note updated successfully" });
        } else {
...
export const getById = async (req: Request, res: Response) => {
    try {
        ...
        const data: any = await prisma.deliveryNote.findFirst({ where: { id: req.body.id }, include: { items: { where: { isDelete: false }, include: { salesOrder: { select: { id: true, invoiceNo: true } } } } } });
        if (data) {
            let items = [];
            ...
            return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: "Invalid deliveryNote id" });
        }
...
export const get = async (req: Request, res: Response) => {
    try {
        ...
        const results = await prisma.deliveryNote.findMany({
            skip: offset,
            take: limit,
            orderBy: {
                id: 'desc',
...
export const deleteDN = async (req: Request, res: Response) => {
    try {
        const userId = req.cookies.id;
        const prisma = getPrismaClient(req.body.db_name);
        const id = req.body.id;

        const lastInvoice = await prisma.deliveryNote.findFirst({ where: { isDelete: false }, orderBy: { id: 'desc' } });
        let lasInvoiceId = 1;
        if (lastInvoice) {
        ...
        if (lasInvoiceId < id) {
            return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: "only last deliveryNote delete" });
        }

        const deleted = await prisma.deliveryNote.update({
            where: {
                id: id
            },
            data: {
        ...
        if (deleted) {
            await activityLog(req.body.db_name, "DELETE", "deliveryNote", req.body, req.cookies.id);
            return res.json({ st: true, statusCode: StatusCodes.OK, msg: "deliveryNote deleted successfully" });
        } else {
            return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: "Invalid deliveryNote id" });
        }
    } catch (e: any) {
...
export const cancel = async (req: Request, res: Response) => {
    try {
        ...
        if (deleted) {
            await activityLog(req.body.db_name, "CANCEL", "deliveryNote", req.body, req.cookies.id);
            return res.json({ st: true, statusCode: StatusCodes.OK, msg: "deliveryNote cancel successfully" });
        } else {
            return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: "Invalid deliveryNote id" });
        }
    } catch (e: any) {
...
export const search = async (req: Request, res: Response) => {
    try {
        ...
        const sales = await prisma.deliveryNote.findMany({
            where: where,
            include: {
                items: {
                    where: {
...
export const download = async (req: Request, res: Response) => {
    try {
        ...
        const prisma = getPrismaClient(req.body.db_name);
        const data: any = await prisma.deliveryNote.findFirst({ where: { id: req.body.id }, include: { items: { where: { isDelete: false } } } });
        if (data) {
            ...
            data.items = items;
            data.Estimation = "DeliveryNote / Estimation";
