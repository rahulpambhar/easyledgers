...
export const getInvoiceNumber = async (req: Request, res: Response) => {
    try {
        ...
        if (voucher) {
            const invoiceNo = await nextInvoice(req.body.db_name, voucher.prefix || "", voucher.numberMethod, req.body.voucherId, "CREDIT");
...
export const insert = async (req: Request, res: Response) => {
    try {
        ...
        if (items.length === 0) {
            return res.json({ st: false, statusCode: StatusCodes.UNAUTHORIZED, msg: 'Invalid sale return item data' });
        }
        ...
        if (voucher.numberMethod === 'Manual') {
            ...
            const salesReturn = await checkInvoiceNo(db_name, "CREDIT", req.body.invoiceNo);
            if (salesReturn) {
                return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: 'Invoice number already exists' });
            }
        } else {
            invoiceNo = await nextInvoice(req.body.db_name, voucher.prefix || "", voucher.numberMethod, req.body.voucherId, "CREDIT");
        }
        ...
        const salesReturn = await prisma.salesReturn.create({ data: data });
        if (salesReturn) {
            const itData = itemData.map((i: any) => { i.salesReturnId = salesReturn.id; return i; });
            await prisma.salesReturnItems.createMany({ data: itData });
            const txData = transactionData.map((i) => { i['refId'] = salesReturn.id; return i; });
            await updateTransaction(db_name, txData);
            await activityLog(db_name, "INSERT", "salesReturn", req.body, req.cookies.id);
            res.json({ st: true, statusCode: StatusCodes.OK, msg: "sales return created successfully" });
        } else {
...
export const update = async (req: Request, res: Response) => {
    try {
        ...
        const prisma = getPrismaClient(db_name);
        const salesReturn = await prisma.salesReturn.findFirst({ where: { id: req.body.id }, include: { items: { where: { isDelete: false } } } })
        if (!salesReturn) {
            return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: 'Invalid delivery note id' });
        }
        ...
        const update = await prisma.salesReturn.update({ where: { id: req.body.id }, data });
        if (update) {
            const oldItemId = salesReturn?.items;
            ...
            if (diff.length > 0) {
                ...
                await prisma.salesReturnItems.updateMany({
                    where: { salesReturnId: req.body.id, id: { in: deleteItemIds } },
                    data: { isDelete: true }
                });
            }
            const itData = itemData.map((i: any) => { i.salesReturnId = salesReturn.id; return i; });
            for (let i in itData) {
                if (!itData[i]?.id && itData[i]?.id === 0) {
                    let dt: any = itData[i];
                    delete dt['id'];
                    await prisma.salesReturnItems.create({
                        data: dt
                    });
                } else {
                    await prisma.salesReturnItems.update({
            ...
            const txData = transactionData.map((i) => { i.refId = salesReturn.id; return i; });
            await updateTransaction(db_name, txData);
            await activityLog(db_name, "UPDATE", "salesReturn", req.body, req.cookies.id);
            res.json({ st: true, statusCode: StatusCodes.OK, msg: "sales return updated successfully" });
        } else {
            res.json({ st: false, statusCode: StatusCodes.OK, msg: "internal server error" });
        }
    } catch (e: any) {
...
export const getById = async (req: Request, res: Response) => {
    try {
        ...
        const data: any = await prisma.salesReturn.findFirst({ where: { id: req.body.id }, include: { items: { where: { isDelete: false }, include: { sales: { select: { id: true, invoiceNo: true } } } } } });
        if (data) {
            let items = [];
...
export const get = async (req: Request, res: Response) => {
    try {
        ...
        const results = await prisma.salesReturn.findMany({
            skip: offset,
            take: limit,
            orderBy: {
                id: 'desc',
...
export const deleteSaleReturn = async (req: Request, res: Response) => {
    try {
        const userId = req.cookies.id;
        const prisma = getPrismaClient(req.body.db_name);
        const id = req.body.id;

        const lastInvoice = await prisma.salesReturn.findFirst({ where: { isDelete: false }, orderBy: { id: 'desc' } });
        let lasInvoiceId = 1;
        if (lastInvoice) {
        ...
        if (lasInvoiceId < id) {
            return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: "only last salesReturn delete" });
        }

        const deleted = await prisma.salesReturn.update({
            where: {
                id: id
            },
            data: {
        ...
        if (deleted) {
            await prisma.transaction.updateMany({ where: { txMode: "CREDIT", refId: id }, data: { isDelete: true } });
            await activityLog(req.body.db_name, "DELETE", "salesReturn", req.body, req.cookies.id);
            return res.json({ st: true, statusCode: StatusCodes.OK, msg: "salesReturn deleted successfully" });
        } else {
            return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: "Invalid salesReturn id" });
        }
    } catch (e: any) {
        return res.json({ st: false, statusCode: StatusCodes.INTERNAL_SERVER_ERROR, msg: e.message });
    }
...
export const cancel = async (req: Request, res: Response) => {
    try {
        ...
        if (deleted) {
            await prisma.transaction.updateMany({ where: { txMode: "CREDIT", refId: id }, data: { isDelete: true } });
            await activityLog(req.body.db_name, "CANCEL", "salesReturn", req.body, req.cookies.id);
            return res.json({ st: true, statusCode: StatusCodes.OK, msg: "salesReturn cancel successfully" });
        } else {
            return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: "Invalid salesReturn id" });
        }
    } catch (e: any) {
...
export const search = async (req: Request, res: Response) => {
    try {
        ...
        return res.json({ st: true, statusCode: StatusCodes.OK, data: salesReturn });
...
export const download = async (req: Request, res: Response) => {
    try {
        ...
        const prisma = getPrismaClient(req.body.db_name);
        const data: any = await prisma.salesReturn.findFirst({ where: { id: req.body.id }, include: { items: { where: { isDelete: false } } } });
        if (data) {
