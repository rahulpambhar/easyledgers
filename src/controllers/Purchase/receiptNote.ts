...
export const getInvoiceNumber = async (req: Request, res: Response) => {
    try {
        ...
        if (voucher) {
            const invoiceNo = await nextInvoice(req.body.db_name, voucher.prefix || "", voucher.numberMethod, req.body.voucherId, "RECEIPT_NOTE");
...
export const insert = async (req: Request, res: Response) => {
    try {
        ...
        if (voucher.numberMethod === 'Manual') {
            ...
            const receiptNote = await checkInvoiceNo(db_name, "RECEIPT_NOTE", req.body.invoiceNo);
            if (receiptNote) {
                return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: 'Invoice number already exists' });
            }
        } else {
            invoiceNo = await nextInvoice(req.body.db_name, voucher.prefix || "", voucher.numberMethod, req.body.voucherId, "RECEIPT_NOTE");
        }

        // const salesOrder = await getDataById(db_name, req.body.purchaseOrderId, "salesOrder", "SalesOrderItems");
        // if (!salesOrder) {
        //     return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: 'Invalid purchaseOrderId' });
        // }
        ...
        for (let x in items) {
            ...
            if (items[x].isExpense) {
                ...
                itemData.push({
                    purchaseOrderId: items[x]?.purchaseOrderId || null,
                    invoiceDate: new Date(req.body.invoiceDate),
        ...
        const receiptNote = await prisma.receiptNote.create({ data: data });
        if (receiptNote) {
            const itData = itemData.map((i: any) => { i.receiptNoteId = receiptNote.id; return i; });
            await prisma.receiptNoteItems.createMany({ data: itData });
            await activityLog(db_name, "INSERT", "receiptNote", req.body, req.cookies.id);
            res.json({ st: true, statusCode: StatusCodes.OK, msg: "delivery note created successfully" });
        } else {
...
export const update = async (req: Request, res: Response) => {
    try {
        ...
        const prisma = getPrismaClient(db_name);
        const getRN = await prisma.receiptNote.findFirst({ where: { id: req.body.id }, include: { items: { where: { isDelete: false } } } })
        if (!getRN) {
            return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: 'Invalid receipt note id' });
        }
        ...
        const receiptNote = await prisma.receiptNote.update({
            where: { id: req.body.id },
            data: data
        });
        if (receiptNote) {
            const oldItemId = getRN?.items;
            ...
            if (diff.length > 0) {
                ...
                await prisma.receiptNoteItems.updateMany({
                    where: { receiptNoteId: req.body.id, id: { in: deleteItemIds } },
                    data: { isDelete: true }
                });
            }
            const itData = itemData.map((i: any) => { i.receiptNoteId = receiptNote.id; return i; });
            for (let i in itData) {
                if (!itData[i]?.id && itData[i]?.id === 0) {
                    let dt: any = itData[i];
                    delete dt['id'];
                    await prisma.receiptNoteItems.create({
                        data: dt
                    });
                } else {
                    await prisma.receiptNoteItems.update({
                        where: { id: itData[i]?.id },
                        data: itData[i],
                    });
                }
            };
            await activityLog(db_name, "UPDATE", "receiptNote", req.body, req.cookies.id);
            res.json({ st: true, statusCode: StatusCodes.OK, msg: "delivery note updated successfully" });
        } else {
            res.json({ st: false, statusCode: StatusCodes.OK, msg: "internal server error" });
        }
    } catch (e: any) {
...
export const getById = async (req: Request, res: Response) => {
    try {
        ...
        const data: any = await prisma.receiptNote.findFirst({ where: { id: req.body.id }, include: { items: { where: { isDelete: false }, include: { purchaseOrder: { select: { id: true, invoiceNo: true } } } } } });
        if (data) {
            let items = [];
...
export const get = async (req: Request, res: Response) => {
    try {
        ...
        const total = await prisma.receiptNote.count({
            where: where
        });

        const results = await prisma.receiptNote.findMany({
            skip: offset,
            take: limit,
            orderBy: {
                id: 'desc',
...
export const deleteRN = async (req: Request, res: Response) => {
    try {
        const userId = req.cookies.id;
        const prisma = getPrismaClient(req.body.db_name);
        const id = req.body.id;

        const lastInvoice = await prisma.receiptNote.findFirst({ where: { isDelete: false }, orderBy: { id: 'desc' } });
        let lasInvoiceId = 1;
        if (lastInvoice) {
        ...
        const deleted = await prisma.receiptNote.update({
            where: {
                id: id
            },
            data: {
        ...
        if (deleted) {
            await activityLog(req.body.db_name, "DELETE", "receiptNote", req.body, req.cookies.id);
            return res.json({ st: true, statusCode: StatusCodes.OK, msg: "receiptNote deleted successfully" });
        } else {
            return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: "Invalid receiptNote id" });
        }
    } catch (e: any) {
        return res.json({ st: false, statusCode: StatusCodes.INTERNAL_SERVER_ERROR, msg: e.message });
    }
...
export const cancel = async (req: Request, res: Response) => {
    try {
        ...
        if (deleted) {
            await activityLog(req.body.db_name, "DELETE", "receiptNote", req.body, req.cookies.id);
            return res.json({ st: true, statusCode: StatusCodes.OK, msg: "receiptNote cancel successfully" });
        } else {
            return res.json({ st: false, statusCode: StatusCodes.BAD_REQUEST, msg: "Invalid receiptNote id" });
        }
    } catch (e: any) {
...
export const search = async (req: Request, res: Response) => {
    try {
        ...
        const sales = await prisma.receiptNote.findMany({
            where: where,
            include: {
                items: {
                    where: {
...
export const download = async (req: Request, res: Response) => {
    try {
        const prisma = getPrismaClient(req.body.db_name);
        const data: any = await prisma.receiptNote.findFirst({ where: { id: req.body.id }, include: { items: { where: { isDelete: false } } } });
        if (data) {
            ...
            for (let row of data.items) {
                if (row.isExpense) {
                    const ledger: any = await prisma.ledger.findFirst({ where: { id: row.itemId as number }, select: { id: true, name: true } });
                    if (ledger) {
                        row.name = ledger?.name;
                    }
                } else {
            ...
            data.Estimation = "Receipt Note / Estimation";
